...
namespace ECommerceCore.Infrastructure.Services
{
        ...
        IOptions<AdminSettings> adminSettings) : IUserService
    {
        ...
        public async Task<IEnumerable<CompanyDto>> GetAllCompaniesAsync()
        {
            try
            {
                var companyEntities = await _unitOfWork.Companies.GetAllAsync();
                var companiesDto = _mapper.Map<IEnumerable<CompanyDto>>(companyEntities);
                return companiesDto;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error while fetching companies");
                throw;
            }
        }
        ...
        private IQueryable<ApplicationUser> ApplySorting(IQueryable<ApplicationUser> query, string sortColumn, string sortDirection)
        {
            ...
            return sortColumn.ToLower() switch
            {
                ...
                _ => isAscending ? query.OrderBy(u => u.Name) : query.OrderByDescending(u => u.Name),
            };
        }
            ...
            try
            {
                using (var transaction = await _dbContext.Database.BeginTransactionAsync())
                {
                    ...
                    if (deletedCount > 0)
                    {
                        ...
                        await _emailService.SendCleanupReportEmailAsync(adminEmails, report);
                    }
